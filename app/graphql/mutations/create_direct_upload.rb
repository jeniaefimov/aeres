...
module Mutations
  class CreateDirectUpload < GraphQL::Schema::Mutation
    ...
    def resolve(input:)
      blob = ActiveStorage::Blob.create_before_direct_upload!(filename: input.filename, byte_size: input.byte_size, checksum: input.checksum, content_type: input.content_type)
