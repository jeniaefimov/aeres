...
class AssociationLoader < ::GraphQL::Batch::Loader
  ...
  def initialize(model, association_schema, scope_model: nil, scopes: nil)
    ...
    @preload_scope = scopes.reduce(scope_model) { |acc, scope| acc.public_send(scope) } if scope_model && scopes
  ...
  def extract_association_id(id_or_hash)
    ...
    raise ArgumentError, "You can only preload exactly one association! You passed: #{id_or_hash}" if id_or_hash.keys.size != 1
